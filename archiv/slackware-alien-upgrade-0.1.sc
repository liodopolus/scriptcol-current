...
pull_onl() {
	...
	for i in $PAR ; do
		cd $SLB
		mkdir -p $i ; cd $i
		rsync -avz --delete --progress --keep-dirlinks --copy-links "$RSYNCURL_SLB/$i/$TYP/$VER" .
...
pull_res_onl() {
	...
	for i in $PAR ; do
		cd $RES
		mkdir -p $i ; cd $i
		rsync -avz --delete --progress --keep-dirlinks --copy-links $RSYNCURL_RES/$i/$TYP/$VER .
		echo -e "\n <download $i>: \t [\033[1;35m  ok  \033[0m]\n"
	done
}
