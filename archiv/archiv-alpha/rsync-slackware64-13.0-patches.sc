...
if [ ! -d ${TOPDIR}/slackware64-$VERSION-patches ]; then
  echo "Target directory ${TOPDIR}/slackware64-$VERSION-patches does not exist!"
  exit 1
fi

cd ${TOPDIR}/slackware64-$VERSION-patches
rsync $1 -avz --delete --exclude "pasture/*" ${RSYNCURL}/linux/slackware/slackware64-$VERSION/patches/packages .
